...
;       - Center of line profile: Parabolic fit of the profile points at and 
;         below the 2nd lowest bisector level.
;       - Determination of "continuum" intensities, intensities near 
;         line center, and  "left" & "right" positions where scan (linearely 
;         interpolated) intersects 
;         y = YMIN(1) + [ycritrel]*(YCONT(1)-YMIN(1)).
;       
...
;       nlte, 1999-02-11  documentation, on_error,2
;       nlte, 2003-01-31 documentation enhanced.       
;-
on_error,2
nparams=n_params()
...
kminfit=1    ; fit parabola for "more accurate minimum" using profile points
;              at and below ycrit(kminfit) 
stepgrob=15  ; 
...
endelse
;
;minimum intensity of spline through normalized intensities:
...
;      minimum of parabola through yint(i11...i22):
...
if nparams gt 8 then begin
   ...
   eqvw=SIMPS(1.-yint(i1:i2))  ; SIMPS: Simpson-intergration, kis_lib
endif
;
if iplot gt 0 then begin
xsort=xr(sort(xr))
